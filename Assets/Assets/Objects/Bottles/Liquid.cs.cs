...
public class Liquid : MonoBehaviour
{
    ...
    {
        fillAmount = Mathf.Clamp(value, 0f, 1f);
        UpdateLiquidVisibility();
    }

    private void UpdateLiquidVisibility()
    {
        if (fillAmount < 0.5f)
            gameObject.SetActive(false); // Hide liquid if fill is less than 0.5
        else
            gameObject.SetActive(true); // Show liquid if fill is 0.5 or more
    }
